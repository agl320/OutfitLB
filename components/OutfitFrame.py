...
class OutfitFrame(tk.CTkFrame):
    def __init__(self, parent, user, *args, **kwargs):
        ...
        self.o_lb_frame = tk.CTkFrame(self)
        self.o_option_fr = tk.CTkFrame(self)
        self.o_view_fr = tk.CTkFrame(self)

        # CTkScrollbar and Listbox
        # self.sb = tk.CTkScrollbar(self.o_lb_frame, orientation=tk.VERTICAL)
        # display current list box of closet
        self.outfit_lb = CTkListbox(
            self.o_lb_frame,
            # command=self.sb.set,
            height=10,
            width=30,
    ...
    def delOutfitPopup(self):
        self.del_outfit_confirm = tk.messagebox.askyesno(
            ...
            message=f"Are you sure you want to delete outfit: {self.outfit_lb.get(self.outfit_lb.curselection())}",
        )

        if self.del_outfit_confirm:
            self.all_outfits.pop(self.outfit_lb.curselection())
            self.outfit_lb.delete(self.outfit_lb.curselection())
            self.updateOutfitList()

    def saveOutfitList(self):
        self.all_outfits[self.outfit_select].set_name(self.edit_n_en.get())
        self.all_outfits[self.outfit_select].set_comb(
        ...
        print(self.all_outfits[self.outfit_lb.curselection()])
    ...
    def editOutfitPopup(self):
        ...
        if self.outfit_lb.curselection():
            self.edit_outfit_window = tk.CTkToplevel(self)
            ...
            self.edit_n_var.set(self.all_outfits[self.outfit_select].get_name())
            ...
            for i, top in enumerate(self.top_lst):
                ...
                print(
                    f"\t{top.get_ID()} ~ {self.all_outfits[self.outfit_select].top.get_ID()}"
                )
                # AFTER ID FIX, USE THIS
                # if (
                #     top.get_ID()
                #     == self.all_outfits[self.outfit_lb.curselection()].top.get_ID()
                # ):
                if (
                    top.get_name()
                    == self.all_outfits[self.outfit_select].top.get_name()
                ):
                    top_val = i
                else:
                    print(f"{top} != {self.all_outfits[self.outfit_select].top}")

            for i, bot in enumerate(self.bottom_lst):
                if bot == self.all_outfits[self.outfit_select].bottom:
                    bot_val = i

            for i, shoe in enumerate(self.shoes_lst):
                if shoe == self.all_outfits[self.outfit_select].shoes:
                    shoe_val = i
            ...
            # cancel button
            self.edit_cancel_b = tk.CTkButton(
                self.edit_outfit_window,
                text="Cancel",
    ...
    def viewOutfit(self):
        ...
        self.o_view_name = tk.CTkLabel(
            self.o_view_fr, textvariable=self.o_view_name_var
        )

        if self.outfit_lb.curselection():
            self.o_view_name_var.set(self.outfit_lb.get(self.outfit_lb.curselection()))
            self.o_view_name.configure(textvariable=self.o_view_name_var)
    ...
    def addOutfitLB(self, window, top_sel=0, bot_sel=0, shoe_sel=0):
        ...
        """
        # CTkScrollbar and Listbox
        # self.sb_sep_top = tk.CTkScrollbar(window, orientation=tk.VERTICAL)
        # display current list box of closet
        self.outfit_sep_top_lb = CTkListbox(
            window,
            exportselection=False,
            # command=self.sb_sep_top.set,
            height=10,
            width=10,
            # activeStyle="dotbox",
            # font=("Helvetica", 10),
        ...
        """
        # CTkScrollbar and Listbox
        # self.sb_sep_bottom = tk.CTkScrollbar(window, orientation=tk.VERTICAL)
        # display current list box of closet
        self.outfit_sep_bottom_lb = CTkListbox(
            window,
            exportselection=False,
            # command=self.sb_sep_bottom.set,
            height=10,
            width=10,
            # activeStyle="dotbox",
            # font=("Helvetica", 10),
        ...
        """
        # CTkScrollbar and Listbox
        # self.sb_sep_shoes = tk.CTkScrollbar(window, orientation=tk.VERTICAL)
        # display current list box of closet
        self.outfit_sep_shoes_lb = CTkListbox(
            window,
            exportselection=False,
            # command=self.sb_sep_shoes.set,
            height=10,
            width=10,
            # activeStyle="dotbox",
            # font=("Helvetica", 10),
    ...
    def changeOutfitList(self, state):
        ...
        outfit_select = self.outfit_lb.curselection()
    ...
    def checkIfRange(self, clothing_lb, lst):
        ...
        try:
            return lst[clothing_lb.curselection()]
        except:
